...
class Proposal(object):
    ...
    def compare_scenarios (self):
        ...
        for hour in range(0,hours_to_print):
            hour_combo = "{}\t".format(str(comparation[0][hour]['_id'].strftime("%d/%m %H:%M")))

            for id,entry in enumerate(comparation):
                hour_combo += "{:^{}}".format(  str(entry[hour]['sum_consumption_proposal']), header_size[id]) + "\t"
            print hour_combo
    ...
    def add_new_scenario(self, name="Default scenario", type="default", collection_name="default"):
        ...
        if type == "cups_increased":
            ...
            new_scenario.add_rule(name="X cups + 1", filter="cups", filter_values="ES0031405879092008YP0F", action="add", action_field="consumption_proposal", action_value="15")

        if type == "cups_erased":
            new_scenario.add_rule(name="X cups x 2", filter="cups", filter_values="ES0031406213108001XL0F", action="multiply", action_field="consumption_proposal", action_value="0")
