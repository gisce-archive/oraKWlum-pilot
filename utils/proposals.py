...
parser.add_option('-s', '--start', dest="input_start_date", help="Start date, format %YYYY-%MM%-%DD")
parser.add_option('-e', '--end', dest="input_end_date", help="End date, format %YYYY-%MM%-%DD")
parser.add_option('-C', '--cpus', dest="cpu", help="Number of CPUs to user for multithread mode")
parser.add_option('-c', '--collection', dest="collection", help="MongoDB collection where to fetch the Consumptions")
...
def main():
    ...
    assert options.input_end_date, "End range date is not provided"
    ...
    DAYS_LIST = datetime_list_from_range(DAY_INITIAL, DAY_LAST)

    Proposal_Massive(days_list=DAYS_LIST, proposal_days_interval=1, CPUs=CPUs)
...
def Proposal_tester(args):
    ...
    print "{}/{} Proposal init for {} - {}".format(id, max, date_start, date_end)
...
def Proposal_Massive(days_list=None, proposal_days_interval=1, type="F1", CPUs=4):
...
def datetime_list_from_range( date_ini, date_end):
    days = []
    date_act = date_ini

    while date_act <= date_end:
        days.append(date_act)
        date_act += timedelta(days=1)
    return days
