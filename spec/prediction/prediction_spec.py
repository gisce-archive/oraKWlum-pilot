...
with description("A Prediction"):
    with it ('must be initialized from a History instance'):
        ...
        history.prediction = Prediction(history=history, start_date=day_ini, end_date=day_fi)

        def create_without_history():
            prediction = Prediction(start_date=day_ini, end_date=day_fi)

        def create_with_NoneHistory():
            prediction = Prediction(history=None,start_date=day_ini, end_date=day_fi)

        expect(create_without_history).to(raise_error(TypeError))
        expect(create_with_NoneHistory).to(raise_error(AssertionError, 'Prediction must be called from a History instance'))
    ...
    with it ('can be initialized with a list of cups followed by a start and end date'):
        ...
        def create_without_list_cups():
            ...
            history = History(start_date=day_ini, end_date=day_fi)
            history.prediction = Prediction(history=history, start_date=day_ini, end_date=day_fi, filter_cups=cups_to_filter)

        expect(create_without_list_cups).to(raise_error(AssertionError, 'cups filter must be None or a list'))
    ...
    with it ('can be processed with a range of dates'):
        ...
        history = History()
        history.prediction = Prediction(start_date=day_ini, end_date=day_fi, history=history)
